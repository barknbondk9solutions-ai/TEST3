...
function getTodayStatus() {
  ...
  if (day === 0) return ""; // Sunday is closed
  ...
  return nowMinutes >= openMinutes && nowMinutes < closeMinutes ? `(Open Now)` : `(Closed)`;
}

function updateStatus() {
  const status = getTodayStatus();
  const day = getEasternTime().getDay();
  if (day >= 1 && day <= 5) document.getElementById("monfri-status").innerText = status;
  else if (day === 6) document.getElementById("sat-status").innerText = status;
  else if (day === 0) document.getElementById("sun-status").innerText = status;
}
